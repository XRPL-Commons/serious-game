<template>
  <div class="rounded-full bg-transparent p-2 transition-all flex justify-center items-center mt-20 flex-col">
    <template v-if="xrplAddress">
      <pre>{{ userNFT }}</pre>
      <pre>{{ ownsNFT }}</pre>
      <div class="nft-item" v-if="NFTExists">
        <div class="m-2 text-center font-title">
          <template v-if="!ownsNFT">
            <div class="ml-2 md:text-2xl text-lg text-gray-600 dark:text-gray-300 mb-4">
              Congratulations!</div>
            ...
            <template v-else>
              One moment while we prepare your NFT...
            </template>
          </template>
...
watch(userNFT, async (newValue: any, oldValue) => {
  if (newValue === '') {
    // need to mint
    const mintResult = await API.createNFT({ xrplAddress })
